# Geass — Detailed Documentation (English)

> Project overview and quick start: [README.md](../../README.md);
> 中文文档见 [docs/zh](../zh/index.md); design reference: [DESIGN.md](../DESIGN.md).

## Index

- [Getting started](getting-started.md): requirements, installation, model & OCR setup, starting, phone connection, remote access, frontend build
- [Features](features.md): feature overview
- [Task planning & memory](tasks-and-memory.md): difficulty self-assessment, planning/verification loop, persistent memory
- [Skill evolution & runtime directory](skills-evolution.md): runtime SKILL layout and idle evolution
- [Manual direct control](manual-control.md): touch, virtual touchpad, virtual keyboard and gestures
- [Trust layer (action preview & privacy masks)](trust-layer.md): ghost actions, tiered review and screenshot redaction
- [Scheduled tasks](schedule.md): run a command at a specified time, with persistence and status events
- [RAG source retrieval](rag.md): register local files/folders, vector/lexical search, source management
- [MCP tools](mcp.md): import/test/manage external MCP tools from the UI and CLI
- [POT reflection system](pot.md): Global-COT and role ROT templates, idle reflection
- [CLI terminal assistant](cli-assistant.md): light/deliberate modes reusing all assets
- [Command line](cli.md): unified entry point, command list, double-confirm reset
- [Resource management (phone)](resources.md): visual management of memory/MCP/RAG/skills/POT/schedule
- [Configuration](configuration.md): precedence, environment variables, CLI and runtime API
- [Project layout & development](development.md): modules, development, known limitations
- [Known issues & capability boundaries](known-issues.md): code-review issues, capability granularity, and limitations by functional area

## Related docs

- [SKILL authoring guide](../../skills/README.md)
- [Design and roadmap](../DESIGN.md)
- [中文详细文档](../zh/index.md)
