...
memory, MCP, RAG, SKILLs and COT/ROT assets.
...
- **light mode (default)**: loads Global-COT, 1-2 relevant ROTs, SKILL
  catalog, RAG injection and memory; tools are read-only assets plus enabled
  MCP tools and confirmed terminal commands; no GUI mouse/keyboard actions.
...
- Slash commands: `/help /mode /deliberate /light /rot list|use /skill
  list|read /rag search /mcp list /cot show /clear /exit`.
