...
2. **扫码（推荐）**：用 `--qr` 启动后，直接用手机相机/微信扫描终端二维码，
   浏览器会自动打开控制界面并完成配对；
3. 也可以手动打开控制台地址（把 `localhost` 换成电脑的局域网 IP，例如
   `http://192.168.1.10:8765`）并输入 Token；
4. 连接成功后即可看到实时屏幕，直接打字或按 🎤 说话下达命令。

> 局域网 IP 可用 `ip addr` 或 `hostname -I` 查看。
> **每次启动都会生成新的随机 Token** 并打印在启动终端里，旧 Token 随即失效。
> 二维码里的是一次性配对码（默认 5 分钟、仅可使用一次），不会暴露长期 Token；
> 过期或已使用后重新运行 `./scripts/run.sh --qr` 即可。
